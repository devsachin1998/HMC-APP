...
export default class QualificationScreenAdminController extends Component<Props, S, SS> {
  // Customizable Area Start
  //   unsubscribe: object;
  ...
  showAlert = (QualificationID) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Yes', onPress:()=> this.deleteuniversity(QualificationID) },
      ],
      { cancelable: false }
    );
  };
  ...
  updateValueById = (articleId) => {
    let updatedDataList = this.state.filterdata.map(article => {
      if (article.QualificationID === articleId) {
  ...
  searchValueById = (Title: string) => {
    let filteredData = this.state.datalist.filter(item => item.QualificationName.toLowerCase().includes(Title.toLowerCase()));
...
getdata = async () => {
  ...
  const responseData = await makeApiCallxml(apiFunctions.QualificationSelect+"?UN1=2&PWD1=2", 'GET', "base");
...
deleteuniversity = async (QualificationID) => {
  this.setState({isLoading:true})
 
  const loginDetails= await getdata("loginDetails");
  let ID =  loginDetails.UserID;
  const res = await makeApiCallxml(apiFunctions.QualificationDelete+`?UN1=2&PWD1=2&QualificationID=${QualificationID}&UserID=${ID}`,'GET',"base");
