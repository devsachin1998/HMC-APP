...
export default class NewsScreenAdminController extends Component<Props, S, SS> {
  // Customizable Area Start
  //   unsubscribe: object;
  ...
  updateValueById = (articleId) => {
    let updatedDataList = this.state.filterdata.map(article => {
      if (article.ScrollNewsId === articleId) {
...
console.log('responseData:::--->headline', updatedTable);
...
deletenews = async (ScrollNewsID) => {
  ...
  const res = await makeApiCallxml(apiFunctions.ScrollNewsDelete+`?UN1=1&PWD1=1&ScrollNewsID=${ScrollNewsID}&UserID=${ID}`,'GET',"admin");
