...
export default class DesignationScreenAdminController extends Component<Props, S, SS> {
  // Customizable Area Start
  //   unsubscribe: object;
  ...
  showAlert = (DesignationID) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Yes', onPress:()=> this.deleteuniversity(DesignationID) },
      ],
      { cancelable: false }
    );
  };
  ...
  updateValueById = (articleId) => {
    let updatedDataList = this.state.filterdata.map(article => {
      if (article.DesignationID === articleId) {
  ...
  searchValueById = (Title: string) => {
    let filteredData = this.state.datalist.filter(item => item.DesignationName.toLowerCase().includes(Title.toLowerCase()));
...
getdata = async () => {
  ...
  const responseData = await makeApiCallxml(apiFunctions.DesignationSelect+"?UN1=2&PWD1=2", 'GET', "base");
...
deleteuniversity = async (DesignationID) => {
  this.setState({isLoading:true})
 
  const loginDetails= await getdata("loginDetails");
  let ID =  loginDetails.UserID;
  const res = await makeApiCallxml(apiFunctions.DesignationDelete+`?UN1=2&PWD1=2&DesignationID=${DesignationID}&UserID=${ID}`,'GET',"base");
