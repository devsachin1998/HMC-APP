...
export default class CountryScreenAdminController extends Component<Props, S, SS> {
  // Customizable Area Start
  //   unsubscribe: object;
  ...
  showAlert = (CountryID) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Yes', onPress:()=> this.deleteuniversity(CountryID) },
      ],
      { cancelable: false }
    );
  };
  ...
  updateValueById = (articleId) => {
    let updatedDataList = this.state.filterdata.map(article => {
      if (article.CountryID === articleId) {
  ...
  searchValueById = (Title: string) => {
    let filteredData = this.state.datalist.filter(item => item.CountryName.toLowerCase().includes(Title.toLowerCase()));
...
getdata = async () => {
  ...
  const responseData = 
  await makeApiCallxml(apiFunctions.CountrySelect+"?UN1=1&PWD1=1", 'GET', "web");
...
deleteuniversity = async (CountryID) => {
  this.setState({isLoading:true})
 
  const loginDetails= await getdata("loginDetails");
  let ID =  loginDetails.UserID;
  const res = await makeApiCallxml(apiFunctions.CountryDelete+`?UN1=2&PWD1=2&CountryID=${CountryID}&UserID=${ID}`,'GET',"base");
